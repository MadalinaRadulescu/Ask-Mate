...
def answer_question(question_id):
    ...
    return render_template("answer_question.html",good_answer_list=good_answer_list,answer_list_dic=answer_list_dic,list_question=list_question, id=question_id)
...
def add_question():
    ...
    if request.method == 'POST':
        for title in data_processing.QUESTION_HEADER:
            ...
            question_dic['submission_time'] = str(int(max([dic['submission_time'] for dic in list_question]))+1000)
    ...
    return render_template("add-question.html",titles=data_processing.QUESTION_HEADER[4:])
...
def add_answer_to_question(question_id):
    ...
    if request.method == 'POST':
        for title in data_processing.ANSWER_HEADER:
            ...
            answer_dic['submission_time'] = str(int(max([dic['submission_time'] for dic in answer_list]))+1000)
    ...
    return render_template("add-answer.html",titles = data_processing.ANSWER_HEADER[4:],id=question_id)
...
def modify_vote(question_id):
    if request.method =="POST":
        ...
        updated_list_dic = data_processing.updatate_voting(question_id,vote, data_processing.get_all_dic(data_processing.QUESITON))
...
def modify_answer_vote(question_id,answer_id):
    if request.method =="POST":
        vote = request.form.get("vote")
        updated_list_dic = data_processing.updatate_voting(answer_id,vote, data_processing.get_all_dic(data_processing.ANSWER))
...
def delete_question(question_id):
    if request.method == "POST":
        updated_dic_list = data_processing.remove_dic_from_list(question_id,data_processing.get_all_dic(data_processing.QUESITON),"id")
        updated_answer_dic_list =data_processing.remove_dic_from_list(question_id,data_processing.get_all_dic(data_processing.ANSWER),"question_id")
...
def delete_answer(question_id,answer_id):
    updated_answer_dic_list =data_processing.remove_dic_from_list(answer_id,data_processing.get_all_dic(data_processing.ANSWER),"id")
    data_processing.rewrite_csv(data_processing.ANSWER,updated_answer_dic_list)
    return redirect("/list")
